...
def stringManipulationPG(file):
    startBook = False #boolean to signify whether to start parsing text, triggered when program sees ***
    totalWords = 0
    fullArray = []
    output = ''
    file = open(file)
    for line in file.readlines():
        if line.find("***") != -1:
            startBook = True     #signifies start of book
        elif startBook and line.find("***") == -1 and len(line) > 1: #when book has begun, line is not blank, and line is not the starter line
            cleanedLine = line.strip()                                #clean line, split, remove punctuation, and add to list
            words = cleanedLine.split()
            modifiedWords = []
            for word in words:
                word = word.translate(string.maketrans("",""), string.punctuation).lower() #used code at following link: http://stackoverflow.com/questions/265960/best-way-to-strip-punctuation-from-a-string-in-python
                if word != "":    #ensures word isn't a blank string
                    modifiedWords = modifiedWords + [word]
                    totalWords = totalWords + 1
            fullArray = fullArray + modifiedWords 
    for element in sorted(fullArray):     #sort array into alphabetical order
       output = output + element+",1\n"   #set up for use in reducer
...
#Input file expected to come from reducer.py output
#returns output in plain text file, 'output.txt'
...
def stringManipulationPG(file):
    ...
    for line in file.readlines():
        if line.find("***") != -1:
            startBook = True
        elif startBook and line.find("***") == -1 and len(line) > 1:
            cleanedLine = line.strip()
            words = cleanedLine.split()
            modifiedWords = []
            for word in words:
